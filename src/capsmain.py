...
import os
import torch
from capsnet import SegCaps, CapsNetBasic, CCCaps
...
def show_n_example(model, n):
    ...
    for i in range(n):
        ...
        for j in range(10):
             print(f"{j}'s... True Count = {round(sum(sum(dat[1][i][j])) / 1000)}, Predicted Count = {round(sum(sum(outputs.cpu().detach().numpy()[0][j])) / 1000)}")
